...
SourcePawn 中已知的结构尺寸 `CNode=12`、`CLeaf=16`、`CBrush=8`、`CBrushSide=8`、`CBoxBrush=48`，在 Windows 目标版本实测确认前全部视为待验证假设。
...
- 初期通过开发配置或控制台开关控制新系统。
- BSP 数据不可用或 brush 解析失败时，默认回退到现有穿越路径。
