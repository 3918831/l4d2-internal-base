...
//
// Purpose: Simplified 4D Vector for L4D2 Portal Mod
//
//=============================================================================//
...
// Define vec_t locally instead of including basetypes.h
typedef float vec_t;

// Simplified Assert macro - replace with standard assert
#ifndef Assert
#ifdef _DEBUG
...
public:
	...
	// Multiply, add, and assign to this (ie: *this = a + b * scalar).
...
// Global constants (moved from const definitions to avoid linker issues)
namespace {
...
//-----------------------------------------------------------------------------
// Vector4D * scalar (returns new Vector4D)
//-----------------------------------------------------------------------------

inline Vector4D operator*(float fl, const Vector4D& v)
{
	return Vector4D(v.x * fl, v.y * fl, v.z * fl, v.w * fl);
}

inline Vector4D operator*(const Vector4D& v, float fl)
{
	return Vector4D(v.x * fl, v.y * fl, v.z * fl, v.w * fl);
}
