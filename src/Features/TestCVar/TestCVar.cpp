...
namespace F {
	...
	void TestCVar::Initialize() {
		...
		// DEBUG: Always print to verify instances exist
		U::LogDebug("TestCVar::Initialize() called\n");
		U::LogDebug("test_var pointer: %p (value: %s)\n", test_var, test_var->GetString());
		U::LogDebug("test_var_float pointer: %p (value: %f)\n", test_var_float, test_var_float->GetFloat());
		U::LogDebug("test_var_bool pointer: %p (value: %d)\n", test_var_bool, test_var_bool->GetInt());

		// Print cvar values to console on init for verification
		if (I::Cvar) {
			U::LogInfo("Test cvars initialized:\n");
			U::LogInfo("  test_var = %s (default: %s)\n",
				test_var->GetString(), test_var->GetDefault());
			U::LogInfo("  test_var_float = %f (default: %s)\n",
				test_var_float->GetFloat(), test_var_float->GetDefault());
			U::LogInfo("  test_var_bool = %d (default: %s)\n",
