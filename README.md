...
| 功能 | 状态 | 说明 |
|------|------|------|
| 传送门创建 | ✅ | 左键创建蓝色传送门，右键创建橙色传送门（需手持马格南） |
| 传送门关闭 | ✅ | 按 R 键触发关闭动画，所有传送门平滑消失（需手持马格南） |
...
**使用方法**：
1. **必须手持马格南手枪 (Magnum)** - 只有持有马格南时才能使用传送门功能
2. 按 **左键** 创建蓝色传送门
3. 按 **右键** 创建橙色传送门
4. 两个传送门会自动建立链接
5. 可以随时重新开枪移动传送门位置
6. 按 **R 键**（换弹键）关闭所有已创建的传送门，触发平滑消失动画

> ⚠️ **武器限制说明**：传送门功能仅当玩家手持 **马格南手枪** (`weapon_pistol_magnum`) 时可用。其他武器（如普通手枪、霰弹枪、近战武器等）均无法触发传送门创建或关闭功能。
...
- ⚠️ **必须手持马格南手枪**，只有持有 Magnum 时传送门功能才会生效
...
### Q: 传送门功能不工作？
A:
1. **确保手持马格南手枪** - 只有 Magnum 才能触发传送门功能
2. 检查是否已安装传送门模型资源（见上方"模型资源安装"部分）
3. 确保在主菜单进入地图，不要在游戏运行中注入
4. 查看控制台输出，确认模型预载成功
